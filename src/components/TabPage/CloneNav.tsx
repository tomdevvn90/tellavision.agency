...
const MenuContainer = Styled.div<{show: any}>`
...
interface NavCloneforTabPropType {
    onUpdateTab: any;
}
const NavCloneforTab: React.FC<NavCloneforTabPropType> = ( props ) => {
    const { onUpdateTab } = props;
    const menu = useSelector((state: RootReducer) => state.landData.menus)?.data.items;
    const currentMenu = useSelector((state: RootReducer) => state.landData.selectedTabBasicDetails);
    const [ show, setShow ] = useState(currentMenu ? (( [ 16, 19 ].includes( currentMenu?.ID ) ) ? 0 : 1) : 1);
    ...
    setTimeout( () => {
         ...
         * 19 => Contact page
         */
        currentMenu ? (( [ 16, 19 ].includes( currentMenu?.ID ) )
            ? setShow( 0 )
            : setShow( 1 )) : setShow(1)
    }, 10 );
...
const MenuClone = ( props ) => {
    ...
    useEffect( () => {
        const updateMenuHeight = () => {
            ...
            liChild.forEach( ( li: HTMLLIElement, index: number ) => {
    ...
    const animate = ( menu: any ) => {
        let menu_nav_item =  (document.querySelector( `.menu-id-${ menu.ID }` ) as HTMLElement)
        let menu_text = (menu_nav_item.querySelector( '.menu-text' ) as HTMLElement)
        ...
        (logo_clone as HTMLElement)?.classList.add( '__clone-logo' );
        (logo_clone as HTMLElement)?.style.setProperty( 'position', 'absolute' );
        (logo_clone as HTMLElement)?.style.setProperty( 'left', `${ logo_pos?.x }px` );
        (logo_clone as HTMLElement)?.style.setProperty( 'top', `${ logo_pos?.y }px` );
        (logo_clone as HTMLElement)?.style.setProperty( 'width', `${ logo_pos?.width }px` );
        (logo_clone as HTMLElement)?.style.setProperty( 'z-index', String(10 ));
        if (logo_clone) {
         ...
         * Hidden menu text
         */
        menu_text.style.setProperty( 'opacity', String(0) );

        /**
         * Hidden content
         */
        try {
            (document.querySelector( '#tab-page .menu-item-main-content' ) as HTMLElement)?.style.setProperty( 'opacity', '0', 'important' )
        } catch( e ) {
         ...
         * Hidden tab title
         */
        let tabTitle = (document.querySelector( '#tab-page .tab-title' ) as HTMLElement);
